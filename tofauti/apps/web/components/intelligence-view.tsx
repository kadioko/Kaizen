...
export function IntelligenceView({ view }: { view: string }) {
  ...
  const { snapshot, connection } = useMarketSnapshot();
  if (!snapshot) return <Loading {...item} />;
...
function VolumeProfile({ snapshot }: { snapshot: MarketSnapshot }) {
  ...
  return <section className="panel rounded-3xl p-6"><p className="text-[10px] font-black uppercase tracking-[.18em] text-zinc-500">Volume by price</p><p className="mt-1 text-lg font-black">Session profile</p><p className="mt-2 text-xs leading-5 text-zinc-400">Demo aggregation from visible OHLCV bars. It is not exchange volume profile data.</p><div className="mt-5 space-y-3">{rows.map(([price, volume]) => <div key={price} className="grid grid-cols-[56px_1fr_42px] items-center gap-2 text-xs"><span className="font-mono text-zinc-400">{price.toFixed(1)}</span><div className="h-3 overflow-hidden rounded-full bg-white/[.06]"><div className="h-full rounded-full bg-violet-400/75" style={{ width: `${volume / maxVolume * 100}%` }} /></div><span className="text-right font-mono text-zinc-500">{volume}</span></div>)}</div></section>;
}
