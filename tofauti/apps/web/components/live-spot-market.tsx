...
function MarketSwitcher({ selectedMarket, setSelectedMarket }: { selectedMarket: LiveSpotSymbol; setSelectedMarket: (market: LiveSpotSymbol) => void }) {
  return <div className="w-full rounded-2xl border border-cyan-300/20 bg-[#0b1118] p-1.5 sm:w-auto"><p className="px-2 pb-1 text-[9px] font-black uppercase tracking-[.16em] text-cyan-200/75">Market</p><div role="group" aria-label="Select live spot market" className="grid grid-cols-2 gap-1 sm:flex">{LIVE_SPOT_SYMBOLS.map((symbol) => {
    const selected = symbol === selectedMarket;
    return <button key={symbol} type="button" aria-pressed={selected} onClick={() => setSelectedMarket(symbol)} className={`rounded-xl border px-3 py-2 text-[10px] font-black tracking-[.1em] transition ${selected ? 'border-cyan-200/45 bg-[#14333c] text-cyan-50 shadow-[inset_0_0_0_1px_rgba(165,243,252,.08)]' : 'border-transparent bg-transparent text-zinc-400 hover:border-white/10 hover:bg-white/[.045] hover:text-zinc-100'}`}>{symbol}</button>;
  })}</div></div>;
}
