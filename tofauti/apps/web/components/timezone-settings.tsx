...
export function TimezoneSettings() {
  ...
  return <section className="panel rounded-3xl border-violet-300/20 p-6"><div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2 text-violet-200"><Clock3 size={18} /><p className="text-[10px] font-black uppercase tracking-[.18em]">Display timezone</p></div><p className="mt-3 text-lg font-black text-white">Show market times in your timezone</p></div><span className="rounded-full border border-violet-300/25 bg-violet-300/10 px-2.5 py-1 text-[9px] font-black tracking-[.12em] text-violet-100">LOCAL PREFERENCE</span></div>
    ...
    <label className="mt-5 block text-[10px] font-black uppercase tracking-[.14em] text-zinc-500">IANA timezone<input value={draftValue} onChange={(event) => { setDraft(event.target.value); setMessage(''); }} className={`mt-2 w-full rounded-xl border bg-black/20 px-3 py-3 font-mono text-sm text-white outline-none transition ${validDraft ? 'border-white/10 focus:border-violet-300/50' : 'border-rose-300/50 focus:border-rose-300'}`} aria-describedby="timezone-help" /></label>
    <div className="mt-3"><p className="text-[9px] font-black uppercase tracking-[.14em] text-zinc-500">Quick choices</p><div className="mt-2 flex flex-wrap gap-2">{options.map((option) => <button key={option} type="button" onClick={() => { setDraft(option); setMessage(''); }} className={`rounded-lg border px-2.5 py-1.5 font-mono text-[10px] transition ${option === draftValue ? 'border-violet-300/40 bg-[#2a2140] text-violet-100' : 'border-white/[.08] bg-[#101018] text-zinc-400 hover:border-violet-300/30 hover:text-zinc-100'}`}>{option}</button>)}</div></div>
