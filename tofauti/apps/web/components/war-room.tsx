...
import { getSnapshot, marketSocketUrl, marketTransportLabel, setDemoScenario, usesBrowserDemo } from '@/lib/market-api';
...
export function WarRoom() {
  ...
  const isReplay = displayConnection === 'browser-demo';
    ...
    <main className="terminal-grid min-h-screen bg-[#0d0b13] text-zinc-100">
      <div className="mx-auto max-w-[1550px] px-4 py-5 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-5 xl:grid-cols-[1.7fr_.85fr]">
          ...
          <div className="grid gap-5"><GoldSpotReference /><section className="panel rounded-3xl p-6"><div className="flex items-center gap-2 text-violet-300"><Bot size={17} /><p className="text-xs font-black uppercase tracking-[.18em]">Analyst boundary</p></div><p className="mt-4 text-lg font-bold">Explanation only. No invented signals.</p><p className="mt-3 text-sm leading-6 text-zinc-400">The local MockAIAnalyst explains only the supplied snapshot; engines own calculations and state changes.</p><div className="mt-5 rounded-xl border border-violet-300/15 bg-violet-300/5 p-3 text-xs leading-5 text-violet-100">Ask next: “Why is Gold {snapshot.alignment.direction.toLowerCase()}?”</div></section></div>
        </div>
